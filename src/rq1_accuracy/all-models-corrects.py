...
for model in models:
    csv_files = glob.glob(f"{DATA_DIR}/{dataset}/{model['name']}/results/{model['best_config']}_fold_*_results.csv")
...
for sample in all_models_corrects:
    ...
    df = pd.read_csv(DATA_DIR / dataset / "data" / f"fold_{fold}_test.csv")
