...
for index, dataset in enumerate(datasets):
    ...
    for model1 in models:
        ...
        model1_results_files = glob.glob(f"{DATA_DIR}/{dataset}/{model1}/results/{model1_best_config}_fold_*_eval_results.json")
        model1_f1_scores = getf1(model1_results_files)
        for model2 in models:
            model2_best_config = get_best_config(model2, dataset)
            model2_results_files = glob.glob(f"{DATA_DIR}/{dataset}/{model2}/results/{model2_best_config}_fold_*_eval_results.json")
