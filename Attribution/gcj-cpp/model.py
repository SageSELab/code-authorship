...
def split_data(fold, type):
    ...
    for index, row in df.iterrows():
        ...
        if len(tokens) <= 512:
            all_rows.append({'id':index, 'code': row['code'], 'author': row['author']})
            continue
        
        chunks = np.array_split(tokens, math.ceil(len(tokens) / 512))
        
        for chunk in chunks:
            all_rows.append({'id':index, 'code': tokenizer.decode(chunk.tolist(), skip_special_tokens=True), 'author': row['author']})
    df_splitted = pd.DataFrame(all_rows, columns = ['id','code','author'])
